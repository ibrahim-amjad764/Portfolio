...
export function AppProvider({ children }) {
  ...
  useEffect(() => {
    const savedTemplate =
      localStorage.getItem(STORAGE_KEYS.template) || "minimal";
    const savedTheme = localStorage.getItem(STORAGE_KEYS.theme) || "dark";
    const savedViews = parseInt(
      localStorage.getItem(STORAGE_KEYS.viewCount) || "0",
      10,
    );

    // Generate random base view count if first visit
    const baseViews =
      savedViews === 0 ? Math.floor(Math.random() * 4000) + 1000 : savedViews;
