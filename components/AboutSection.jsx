  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm text-[var(--portfolio-muted)] font-medium mt-0.5">{label}</p>
      </div>
    </motion.div>
  );
}
...
export default function AboutSection({ data }) {
  ...
  const stats = [
    { value: data.stats.projects, label: 'Projects Shipped', icon: Layers, delay: 0.1 },
    { value: data.stats.yearsExperience, label: 'Years of Experience', icon: Briefcase, delay: 0.2 },
    { value: data.stats.skillsCount, label: 'Technologies Mastered', icon: Code2, delay: 0.3 },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <FadeInSection direction="right" className="space-y-4">
            ...
            <motion.div
              ...
              style={{
                background: 'linear-gradient(135deg, var(--portfolio-accent)/10, var(--portfolio-accent-2)/10)',
                borderColor: 'var(--portfolio-accent)',
                borderWidth: '1px',
              }}
            >
              <p className="text-sm text-[var(--portfolio-muted)] font-medium mb-1">Fun fact</p>
              <p className={`text-[var(--portfolio-text)] font-semibold ${isDarkDev ? 'font-mono' : ''}`}>
