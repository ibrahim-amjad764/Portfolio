...
function SkillCard({ skill, index, animate }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className={templateConfig.skillCardClass}
    >
      <div className="flex items-center gap-3 mb-3">

  {skill.icon && (
    <skill.icon
      className="text-2xl text-[var(--portfolio-accent)] flex-shrink-0"
    />
  )}

  <div className="min-w-0">
    <p className={`font-semibold text-sm text-[var(--portfolio-text)] truncate
      ${isDarkDev ? 'font-mono' : ''}`}>
      {skill.name}
    </p>
    ...
    {skill.proficiency}%
      ...
      <div className="progress-bar-track" role="progressbar"
        ...
        <motion.div
          ...
          transition={{ duration: 1, delay: 0.1 + index * 0.05, ease: [0.25, 1, 0.5, 1] }}
        />
      </div>
    </motion.div>
  );
...
export default function SkillsSection({ skills = [] }) {
  ...
  return (
    <section id="skills" className={`section-padding ${templateConfig.sectionBg}`} aria-label="Skills">
      <div className="container-max" ref={ref}>
        ...
        <FadeInSection className="mb-12 sm:mb-16">
          ...
          <p className="mt-3 text-[var(--portfolio-muted)] max-w-xl">
            {isDarkDev ? '// ' : ''}Technologies I work with daily and have mastered over the years.
          </p>
        </FadeInSection>
        ...
        >
          <AnimatePresence mode="popLayout">
            {filtered.map((skill, i) => (
              <SkillCard key={skill.name} skill={skill} index={i} animate={inView} />
            ))}
          </AnimatePresence>
        </motion.div>
