...
function ExperienceCard({ exp, index }) {
  ...
  return (
    <div className={`relative flex gap-6 ${isEven ? 'flex-row' : 'flex-row'} items-start`}>
      {/* Timeline dot */}
      <div className="flex flex-col items-center flex-shrink-0 mt-1" aria-hidden="true">
        ...
        {/* Vertical line */}
        <div className="timeline-line w-0.5 flex-1 mt-2 min-h-[2rem]" aria-hidden="true" />
      </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              {exp.companyUrl ? (
                ...
                <span className="text-[var(--portfolio-accent)] font-semibold text-sm">{exp.company}</span>
              )}
              <span className="text-xs text-[var(--portfolio-muted)] px-2 py-0.5 rounded-full
          ...
          {exp.points.map((point, i) => (
            <li key={i} className="flex items-start gap-2.5">
              <span
                ...
                aria-hidden="true"
              />
              <span className={`text-sm text-[var(--portfolio-muted)] leading-relaxed
...
export default function ExperienceSection({ experience = [] }) {
  ...
  return (
    <section id="experience" className={`section-padding ${templateConfig.sectionBg}`} aria-label="Work experience">
      <div className="container-max">
        <FadeInSection className="mb-12 sm:mb-16">
          ...
          <p className="mt-3 text-[var(--portfolio-muted)] max-w-xl">
            {isDarkDev ? '// ' : ''}My professional journey and the companies I&apos;ve had the privilege to work with.
          </p>
        </FadeInSection>
