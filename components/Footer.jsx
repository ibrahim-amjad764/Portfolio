      ...
      >
        <div className="container-max px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            {/* Left — copyright */}
            <p
              className={`text-sm text-[var(--portfolio-muted)] text-center sm:text-left
              ${isDarkDev ? "font-mono" : ""}`}
            >
              © {year}{" "}
              <span className="text-[var(--portfolio-text)] font-semibold">
                {data.name}
              </span>
                ...
                {socials.map(({ href, label, icon: Icon }) => (
                  <li key={label}>
                    {href.startsWith("mailto:") ? (
                      ...
                      <HoverPeek url={href} className="block">
                        <motion.a
                          href={href}
                          target="_blank"
